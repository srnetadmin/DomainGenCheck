...
class DomainGenerator:
    ...
    def generate_domain_variations(self, domain: str) -> Generator[str, None, None]:
        ...
        techniques = [
            ...
            (
                self.config.enable_character_repetition,
                self._generate_repetition_variants,
            ),
            (
                self.config.enable_character_substitution,
                self._generate_substitution_variants,
            ),
            ...
            (
                self.config.enable_subdomain_variations,
                self._generate_subdomain_variants,
            ),
        ]
            ...
            try:
                for variant_domain in technique_func(base_domain):
                    ...
                    for tld in self._get_relevant_tlds(original_tld):
                        ...
                        if (
                            full_domain not in seen_domains
                            and self._is_valid_domain_name(full_domain)
                            and full_domain != domain
                        ):
    ...
    def _generate_substitution_variants(
        self, domain: str
