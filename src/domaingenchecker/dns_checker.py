...
class DNSChecker:
    ...
    def __init__(self, config: DNSConfig):
        ...
        logger.info(f"Initialized DNSChecker with {config.concurrent_limit} concurrent limit")
    
    async def check_domains(self, domains: List[str], use_cache: bool = True) -> List[DNSResult]:
    ...
    async def check_domain(self, domain: str, use_cache: bool = True) -> DNSResult:
        ...
        logger.debug(f"DNS check for {domain}: {dns_result.status.value} ({dns_result.response_time:.3f}s)")
        return dns_result
